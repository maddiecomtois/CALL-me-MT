...
class Translator {
    ...
    constructor(authKey, options) {
        ...
        this.httpClient = new client_1.HttpClient(serverUrl, headers, maxRetries, minTimeout);
    }
    /**
    ...
    async getDocumentStatus(handle) {
        ...
        const { statusCode, content } = await this.httpClient.sendRequestWithBackoff('GET', `/v2/document/${handle.documentId}`, { data });
    ...
    async internalDownloadDocument(handle, outputStream) {
        const data = new url_1.URLSearchParams({ document_key: handle.documentKey });
        const { statusCode, content } = await this.httpClient.sendRequestWithBackoff('GET', `/v2/document/${handle.documentId}/result`, { data }, true);
        await checkStatusCode(statusCode, content, false, true);
